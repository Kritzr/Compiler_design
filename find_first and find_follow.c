...
char predictive[10][20][10] = {{{0}}}; // Updated dimensions for clarity
...
void construct_predictive_parser() {
    for (int i = 0; i < np; i++) {
        char lhs = mat[i][0];
        for (int j = 0; first[lhs][j] != '\0'; j++) {
            char first_symbol = first[lhs][j];

            if (first_symbol == '~') {
                for (int k = 0; follow[lhs][k] != '\0'; k++) {
                    snprintf(predictive[lhs][follow[lhs][k]], sizeof(predictive[lhs][follow[lhs][k]]), "E");
                }
            } else {
                strncat(predictive[lhs][first_symbol], mat[i], strlen(mat[i]) - 1);
            }
        }
    }
}
...
int main() {
    ...
    for (int i = 0; i < np; i++) {
        ...
        mat[i][strlen(production) - 3 + 1] = '#'; // Ensure null termination
        mat[i][strlen(production) - 3 + 2] = '\0';
    }

    follow[start_symbol][0] = '$';

    for (int i = 0; i < np; i++) {
        find_first(i, 1); // Start from index 1
    }
    ...
    for (int i = 0; i < symbol_count; i++) {
        char non_terminal = symbols[i];
        printf("\n%c -> ", non_terminal);
        for (int j = 0; j < symbol_count; j++) {
            char terminal = symbols[j];
            if (predictive[non_terminal][terminal][0] != '\0') {
                printf("%c: %s | ", terminal, predictive[non_terminal][terminal]);
            }
        }
    }
